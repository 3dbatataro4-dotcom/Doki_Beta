window.ChatSystem = {
    ...
    renderMoments: function() {
        ...
        // 根據路線插入特殊動態
        if (this.gameInstance.state.route === "lynn") {
            if(!window.MOMENTS.find(m=>m.postId==="m_peter_special")) window.MOMENTS.unshift({ postId: "m_peter_special", id: "peter", name: "彼得", avatar: "peter", content: "終於找到了。我的藥。我的貓。全世界最乾淨的存在。 #FoundYou #MyCat", time: "剛剛" });
        } else if (this.gameInstance.state.route === "jornona") {
            if(!window.MOMENTS.find(m=>m.postId==="m_lanlan_special")) window.MOMENTS.unshift({ postId: "m_lanlan_special", id: "lanlan", name: "蘭蘭", avatar: "lanlan", content: "老婆回來了呀！！！開心到想把學校燒了助興！🔥🔥🔥 #Love #Wife", time: "剛剛" });
        } else if (this.gameInstance.state.route === "melas") {
            if(!window.MOMENTS.find(m=>m.postId==="m_ora_special")) window.MOMENTS.unshift({ postId: "m_ora_special", id: "ora", name: "奧拉", avatar: "ora", content: "變量回歸。實驗重啟。這一次，數據會很有趣。", time: "剛剛" });
        }

        window.MOMENTS.forEach(post => {
            const div = document.createElement("div");
            div.className = "moment-post";
            let avatarUrl = window.ASSETS.avatar[post.id];
            div.innerHTML = `
                ...
                <div class="moment-actions">
                    <div class="moment-like" onclick="this.classList.toggle('liked'); document.getElementById('se-click').play();">♥ 讚</div>
                    <div style="font-size:0.8em;">💬</div>
                    <input type="text" id="input-${post.postId}" class="moment-comment-input" placeholder="寫下留言..." onkeydown="if(event.key==='Enter') window.Game.postComment(this, '${post.id}', '${post.postId}')">
                    <div class="moment-send-btn" onclick="window.Game.postComment(document.getElementById('input-${post.postId}'), '${post.id}', '${post.postId}')">➤</div>
                </div>
        ...
        let newButton = `<div class="choice-btn" style="text-align:center; margin-top:10px;" onclick="alert('功能開發中')">發布動態</div>`;
        feed.innerHTML += newButton;
    },
    ...
    initContactList: function() {
        ...
        // 默認顯示群組
        if (this.gameInstance.state.currentChatTarget === 'group') {
            this.openChat('group', '身心靈自助研究社');
        } else {
